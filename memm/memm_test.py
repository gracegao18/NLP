...
prob_start = load_model(os.path.join(os.path.dirname(__file__), "data\prob_start.py"))
prob_emit = load_model(os.path.join(os.path.dirname(__file__), "data\prob_emit.py"))
...
def viterbi(obs, states, start_p, emit_p):  # 维特比算法（一种递归算法）
	...
	for i in range(1,lenght):
		...
		for static in states:
			list = []
			max = 0
			for key in alllines[i-1].keys():  # [当前预测状态,上一个状态值,观测值]
