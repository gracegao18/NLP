...
class BiLSTM_CRF(nn.Module):
    ...
    # def __init__(self, vocab_size, tag_to_ix, embedding_dim, hidden_dim, crf, target_size):
    def __init__(self, vocab_size, embedding_dim, hidden_dim, crf, target_size):
...
if __name__ == '__main__':
    ...
    model = BiLSTM_CRF(len(word_to_ix), EMBEDDING_DIM, HIDDEN_DIM, crf, len(tag_to_ix))
    # 模型训练优化器
    ...
    with torch.no_grad():
        ...
        out = out.reshape(len(precheck_sent),1,-1)
        print(crf.decode(out))

    # Make sure prepare_sequence from earlier in the LSTM section is loaded
    for epoch in range(300):  # 循环次数，这里是简单样本，正常情况下我们不需要300轮循环训练
        for sentence, tags in training_data:
            # Step 1. 清除累计的梯度值
            model.zero_grad()
            ...
            # Step 3. 运行前向运算
            out = model(sentence_in).reshape(len(sentence_in), 1, -1)
            targets = targets.reshape(len(sentence_in), -1)
            loss = -1 * crf(out, targets)
